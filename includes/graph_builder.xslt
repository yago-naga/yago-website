	...
	<xsl:template name="makeTaxonomy">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:if test="$numberOfObjects - number($isClass)*2=0">
					<text text-anchor="middle" x="{$x}" y="{$y + $fontSize* 0.2}" font-size="{$fontSize}" fill="gray">Not all entities</text>
					<text text-anchor="middle" x="{$x}" y="{$y + $fontSize* 1.5}" font-size="{$fontSize}" fill="gray">have been loaded.</text>
				</xsl:if>
