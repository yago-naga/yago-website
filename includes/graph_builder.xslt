...
<!--- Transforms a SPARQL query result about an entity into an SVG visualization. The query takes the following form:
...
<xsl:stylesheet
    ...
    <xsl:variable name="fontSize" select="20"/>
    ...
    <xsl:template name="makeTaxonomy">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <text x="{$x - $fontSize*string-length($entity)*0.5 div 2}" y="{$y+$fontSize}" font-size="{$fontSize}">
                ...
                <xsl:for-each select="$facts">
                    ...
                    <xsl:choose>
                        <xsl:when test="position()&lt; $numberOfFacts div 2">
                            ...
                            <text x="{$x+($radius - $fontSize*10)}" y="{$y - $fontSize*0.2}" transform="rotate({180 div ($numberOfFacts + 1)*position()} {$x} {$y})" font-size="{$fontSize}" fill="blue">
                                <xsl:value-of select="$predicate" />
                            </text>
                        </xsl:when>
                        <xsl:otherwise>
                            <text x="{$x - $radius - string-length($object)*$fontSize*0.5}" y="{$y+$fontSize*0.3}" transform="rotate({-180 div ($numberOfFacts + 1)*($numberOfFacts - position()+1)} {$x} {$y})" font-size="{$fontSize}"  fill="blue">
                                <xsl:value-of select="$object" />
                            </text>
